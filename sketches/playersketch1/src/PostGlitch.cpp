...
void PostGlitch::setup(ofFbo *buffer_) {
	...
	plane.mapTexCoords(0, 0, buffer_size.x, buffer_size.y);
}

void PostGlitch::setFbo(ofFbo *buffer_) {
	targetBuffer = buffer_;
	buffer_size.set(buffer_->getWidth(), buffer_->getHeight());
	ShadingBuffer.allocate(buffer_size.x, buffer_size.y);
}

void PostGlitch::setFx(ofxPostGlitchType type_, bool enabled) {
	bShading[type_] = enabled;
}

void PostGlitch::toggleFx(ofxPostGlitchType type_) {
	bShading[type_] ^= true;
}

bool PostGlitch::getFx(ofxPostGlitchType type_) {
	return bShading[type_];
}

void PostGlitch::generateFx() {
	...
	for (int i = 0; i < GLITCH_NUM; i++) {
		if (bShading[i]) {
			...
			shader[i].setUniformTexture("image", targetBuffer->getTextureReference(), 0);
